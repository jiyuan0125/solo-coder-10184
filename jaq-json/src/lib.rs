...
impl jaq_core::ValT for Val {
    ...
    fn range(self, range: val::Range<&Self>) -> ValR {
        let fs = |b: Bytes, range, skip_take: SkipTakeFn| {
            Self::range_int(range)
                .and_then(|range_char| skip_take(range_char, &b))
        ...
        match self {
            ...
            Val::Arr(a) => Self::range_int(range)
                .and_then(|range| skip_take(range, a.len()))
    ...
    fn map_range<'a, I: Iterator<Item = ValX<'a>>>(
        ...
        let fs = |b: Bytes, range, skip_take: SkipTakeFn, from: ValBytesFn, into: BytesValFn| {
            let (skip, take) = match Self::range_int(range).and_then(|range| skip_take(range, &b)) {
                Ok(result) => result,
        ...
        match self {
            Val::Arr(ref mut a) => {
                let (skip, take) = match Self::range_int(range).and_then(|range| skip_take(range, a.len())) {
                    Ok(result) => result,
                    Err(e) => return opt.fail(self, |_| Exn::from(e)),
                };
...
type SkipTakeFn = fn(val::Range<num::PosUsize>, &[u8]) -> Result<(usize, usize), Error>;

fn skip_take(range: val::Range<num::PosUsize>, len: usize) -> Result<(usize, usize), Error> {
    let from = abs_bound(range.start, len, 0)?;
    let upto = abs_bound(range.end, len, len)?;
    Ok((from, upto.saturating_sub(from)))
}

fn skip_take_bytes(range: val::Range<num::PosUsize>, b: &[u8]) -> Result<(usize, usize), Error> {
    skip_take(range, b.len())
}

fn skip_take_chars(range: val::Range<num::PosUsize>, b: &[u8]) -> Result<(usize, usize), Error> {
    ...
    Ok((from_byte, upto_byte.saturating_sub(from_byte)))
}
