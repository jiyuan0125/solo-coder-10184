...
impl Num {
    ...
    pub fn from_str_radix(i: &str, radix: u32) -> Option<Self> {
        // Strip underscore separators for isize parsing
        let stripped: String = i.chars().filter(|c| *c != '_').collect();
        let int = || isize::from_str_radix(&stripped, radix).ok().map(Num::Int);
        let big = || bigint_from_str_radix(&stripped, radix).map(Self::big_int);
        int().or_else(big)
    }

    /// Try to parse a decimal string to a [`Self::Float`], else return NaN.
    pub fn from_dec_str(n: &str) -> Self {
        // Strip underscore separators for decimal parsing
        let stripped: String = n.chars().filter(|c| *c != '_').collect();
        stripped.parse().map_or(Self::Float(f64::NAN), Self::Float)
    }
...
fn biguint_from_str_radix(s: &str, radix: u32) -> Option<BigUint> {
    ...
    // normalize all characters to plain digit values, skip underscores
    ...
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            ...
            Self::Float(f) => {
                ...
                if f.is_nan() {
                    state.write_u8(0);
                } else if f.is_sign_positive() && f.is_infinite() {
                    state.write_u8(1);
                } else if f.is_sign_negative() && f.is_infinite() {
                    state.write_u8(2);
                } else if *f == 0.0 {
                    0.0f64.to_ne_bytes().hash(state);
                } else {
...
fn hash_nums() {
    ...
    // Non-finite floats should have distinct hashes
    use core::f64::{INFINITY, NAN, NEG_INFINITY};
    assert_ne!(h(Num::Float(NAN)), h(Num::Float(INFINITY)));
    assert_ne!(h(Num::Float(NAN)), h(Num::Float(NEG_INFINITY)));
    assert_ne!(h(Num::Float(INFINITY)), h(Num::Float(NEG_INFINITY)));
    // NaN should equal itself for hashing purposes
    assert_eq!(h(Num::Float(NAN)), h(Num::Float(NAN)));
    assert_eq!(h(Num::Float(INFINITY)), h(Num::Float(INFINITY)));
    assert_eq!(h(Num::Float(NEG_INFINITY)), h(Num::Float(NEG_INFINITY)));
}
...
fn float_cmp(left: f64, right: f64) -> Ordering {
    if left == 0. && right == 0. {
        ...
        // so we bite the bullet and handle this like in jq: NaN == NaN
        Ordering::Equal
    } else if left.is_nan() {
        // NaN is greater than any non-NaN value in jq
        Ordering::Greater
    } else if right.is_nan() {
        // any non-NaN value is less than NaN in jq
        Ordering::Less
    } else {
...
#[test]
fn nan_comparison_symmetric() {
    use core::cmp::Ordering;
    use core::f64::{INFINITY, NAN, NEG_INFINITY};

    // NaN should equal NaN
    assert_eq!(float_cmp(NAN, NAN), Ordering::Equal);
    assert!(float_eq(NAN, NAN));

    // NaN should be greater than any non-NaN value
    assert_eq!(float_cmp(NAN, 0.0), Ordering::Greater);
    assert_eq!(float_cmp(NAN, INFINITY), Ordering::Greater);
    assert_eq!(float_cmp(NAN, NEG_INFINITY), Ordering::Greater);

    // Any non-NaN value should be less than NaN
    assert_eq!(float_cmp(0.0, NAN), Ordering::Less);
    assert_eq!(float_cmp(INFINITY, NAN), Ordering::Less);
    assert_eq!(float_cmp(NEG_INFINITY, NAN), Ordering::Less);

    // Comparison should be symmetric
    assert_eq!(float_cmp(NAN, 1.0).reverse(), float_cmp(1.0, NAN));
    assert_eq!(float_cmp(1.0, NAN).reverse(), float_cmp(NAN, 1.0));
}

#[test]
fn underscore_separated_integers() {
    // Test underscore-separated integers parse correctly
    assert_eq!(Num::from_str_radix("1_000_000", 10), Some(Num::Int(1_000_000)));
    assert_eq!(Num::from_str_radix("1_000", 10), Some(Num::Int(1000)));
    assert_eq!(Num::from_str_radix("1_0_0", 10), Some(Num::Int(100)));

    // Test that they work as BigInt too
    let big = Num::from_str("1_000_000_000_000_000_000_000");
    assert!(big.is_int());
    assert_eq!(big.to_string(), "1000000000000000000000");
}

#[test]
fn non_finite_float_equality() {
    use core::f64::{INFINITY, NAN, NEG_INFINITY};

    // NaN should equal NaN
    assert_eq!(Num::Float(NAN), Num::Float(NAN));
    // Infinities should equal themselves
    assert_eq!(Num::Float(INFINITY), Num::Float(INFINITY));
    assert_eq!(Num::Float(NEG_INFINITY), Num::Float(NEG_INFINITY));
    // Non-finite values should not equal each other
    assert_ne!(Num::Float(NAN), Num::Float(INFINITY));
    assert_ne!(Num::Float(NAN), Num::Float(NEG_INFINITY));
    assert_ne!(Num::Float(INFINITY), Num::Float(NEG_INFINITY));
}

#[test]
fn non_finite_float_hash_distinct() {
    use core::f64::{INFINITY, NAN, NEG_INFINITY};
    use core::hash::BuildHasher;
    let h = |n: &Num| foldhash::fast::FixedState::with_seed(42).hash_one(n);

    // NaN, +INFINITY, -INFINITY should all have different hashes
    assert_ne!(h(&Num::Float(NAN)), h(&Num::Float(INFINITY)));
    assert_ne!(h(&Num::Float(NAN)), h(&Num::Float(NEG_INFINITY)));
    assert_ne!(h(&Num::Float(INFINITY)), h(&Num::Float(NEG_INFINITY)));
}
...
#[test]
fn zero_hash_consistent() {
    use core::hash::BuildHasher;
    let h = |n: &Num| foldhash::fast::FixedState::with_seed(42).hash_one(n);

    // +0 and -0 should have the same hash (since they are equal)
    assert_eq!(Num::Float(0.0), Num::Float(-0.0));
    assert_eq!(h(&Num::Float(0.0)), h(&Num::Float(-0.0)));

    // Int(0) should also hash the same as Float(0.0)
    assert_eq!(h(&Num::Int(0)), h(&Num::Float(0.0)));
}

#[test]
fn bigint_overflow_hash_distinct_from_float_inf() {
    use core::f64::INFINITY;
    use core::hash::BuildHasher;
    use num_bigint::BigInt;
    let h = |n: &Num| foldhash::fast::FixedState::with_seed(42).hash_one(n);

    // A very large BigInt that overflows to infinity when converted to f64
    let huge = Num::big_int(BigInt::from(1) << 2048);
    assert!(!huge.as_f64().is_finite());

    // BigInt overflow to infinity should NOT have the same hash as Float(INFINITY)
    assert_ne!(h(&huge), h(&Num::Float(INFINITY)));
}

#[test]
fn underscore_separated_decimals() {
    // Test underscore-separated decimal numbers parse correctly
    let n = Num::from_dec_str("1_000.5");
    assert_eq!(n.as_f64(), 1000.5);

    let n = Num::from_dec_str("1_000_000.123_456");
    assert_eq!(n.as_f64(), 1000000.123456);

    let n = Num::from_dec_str("1.5e1_0");
    assert_eq!(n.as_f64(), 1.5e10);
}

#[test]
fn float_display_matches_expected() {
    use core::f64::{INFINITY, NAN, NEG_INFINITY};

    assert_eq!(Num::Float(NAN).to_string(), "NaN");
    assert_eq!(Num::Float(INFINITY).to_string(), "Infinity");
    assert_eq!(Num::Float(NEG_INFINITY).to_string(), "-Infinity");
    assert_eq!(Num::Float(0.0).to_string(), "0.0");
    assert_eq!(Num::Float(-0.0).to_string(), "-0.0");
    assert_eq!(Num::Float(1.5).to_string(), "1.5");
    assert_eq!(Num::Float(1e100).to_string(), "1e+100");
}
