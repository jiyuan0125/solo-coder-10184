...
#[test]
fn try_catch_update() {
    // Update on try-catch block should work
    give(json!({"a": 1}), "(try .a catch 0) |= . + 1", json!(2));
    give(json!({}), "(try .a catch 0) |= . + 1", json!(1));
    give(json!({"a": 5}), "(try .a catch 0) |= . * 2", json!(10));
}
...
fn underscore_integers() {
    // Underscore-separated integers should parse as numbers
    give(json!(null), "1_000_000", json!(1000000));
    give(json!(null), "1_000 + 2_000", json!(3000));
    give(json!(null), "1_000_000_000_000_000_000_000 | type", json!("number"));
    give(json!(null), "1.5e1_0", json!(1.5e10));
}

#[test]
fn nan_comparison_symmetric() {
    // NaN comparison should be symmetric
    give(json!(null), "nan < 1", json!(false));
    give(json!(null), "1 < nan", json!(true));
    give(json!(null), "nan > 1", json!(true));
    give(json!(null), "1 > nan", json!(false));
    give(json!(null), "nan == nan", json!(true));
}
...
#[test]
fn array_slicing_jq_compatible() {
    // Array slicing behavior should match jq
    give(json!([1, 2, 3, 4]), ".[-100:5]", json!([1, 2, 3, 4]));
    give(json!([1, 2, 3, 4]), ".[:-100]", json!([]));
    give(json!([1, 2, 3, 4]), ".[-100:-50]", json!([]));
    give(json!([1, 2, 3, 4]), ".[-5:-3]", json!([1]));
    give(json!([1, 2, 3, 4]), ".[1:100]", json!([2, 3, 4]));
}
...
fn zero_equality() {
    ...
    give(json!(null), "0 == -0", json!(true));
    give(json!(null), "0.0 == -0.0", json!(true));
    give(json!(null), "[0, -0] | unique | length", json!(1));
}
