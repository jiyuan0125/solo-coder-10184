...
///
/// All exceptions are converted into regular errors, including halt exceptions.
/// This function is safe to use in embedded contexts and will not terminate the process.
///
...
{
    v.map_err(|e| e.into_err())
}

/// Convert a value exception [`ValX`] into a value result [`ValR`]
/// without exiting the process on halt exceptions.
///
/// This is a safe alternative to [`unwrap_valr`] for library users
/// who want to handle all exceptions gracefully.
///
/// Unlike [`unwrap_valr`], this function converts halt exceptions
/// into regular errors instead of exiting the process or panicking.
///
/// ~~~
